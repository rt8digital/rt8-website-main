...
export default function LiquidEther({
	...
	onReady, // Destructure onReady callback
...
`;
		...
		class WebGLManager implements LiquidEtherWebGL {
			...
			init() {
				...
				// Notify that the background is ready
				if (onReady) {
					setTimeout(onReady, 100); // Small delay to ensure DOM is ready
				}
			}
			resize() {
				Common.resize();
		...
		const webgl = new WebGLManager({
			...
			autoResumeDelay,
			autoRampDuration,
			onReady, // Pass onReady callback to WebGLManager
		});
